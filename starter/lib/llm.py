...
class LLM:
    def __init__(
        ...
        if not self.api_key:
            raise ValueError("No OpenAI API key found. Set it via env or pass explicitly.")

        # Check for Vocareum key
        is_vocareum = self.api_key.startswith("voc-")
        api_base = "https://openai.vocareum.com/v1" if is_vocareum else None

        if is_vocareum:
            print("📡 Vocareum key detected — using Vocareum API endpoint.")
